...
import rospy
from script.PI_Controller import PID_Controller
import tf
...
def PressureCallback(data):
    ...
    yd_init = 0
    yd_final = 1
    #t_final = 3.5  # yaw timmmme 
    psid_init = 0
    psid_final = 1.5708    # psi_angle = 90 degree

    #t_final = 6.4     surge time 
    t_final = 3.5  #6
    ...
    if(enable_depth):  
        ...
        if(test_Model):
            ...
            if initial_time_bool == True:
                initial_time = rospy.Time.now().to_sec()  # calculate the initial time (s)
            ...
            #Depth control :
            heave_force = PID_Controller(0.3, depth_wrt_startup, 2, 0, 0, e0_z, I0_z, 0.05, 0.19, r = None)
            
            #Cap control using PI controller
            ...
            if motion == "surge" :
                ...
                xd, ud, ud_d = quintic_Trajectory(xd_init , xd_final, current_time, t_final) 
                Pos = np.array([xd, 0, 0, 0, 0, 0])
                Speed_B = np.array([ud, 0, 0, 0, 0, 0])
                Acc_B = np.array([ud_d, 0, 0, 0, 0, 0])

                # Pos = np.array([first_joint_pos[cont], 0, 0, 0, 0, 0])
                # Speed_B = np.array([first_joint_vel[cont], 0, 0, 0, 0, 0])
                # Acc_B = np.array([first_joint_acc[cont], 0, 0, 0, 0, 0])
                # cont += 1 
                
                if cont == len(first_joint_pos):
                    cont -= 1
                tau = torque_rov(Acc_B, Pos, Speed_B)
                surge_pwm = PWM_Cmd(tau[0])
                setOverrideRCIN(1500, 1500, heave_pwm, yaw_pwm ,1500, 1500)

            if motion == "sway":
                # # #Desired position/velocity and acceleration :
                yd, vd, vd_d = quintic_Trajectory(yd_init, yd_final, current_time, t_final) 
                Pos = np.array([0, yd, 0, 0, 0, 0])
                Speed_B = np.array([0, vd, 0, 0, 0, 0])
                Acc_B = np.array([0,vd_d, 0, 0, 0, 0])
                tau = torque_rov(Acc_B, Pos, Speed_B)
                sway_pwm = PWM_Cmd(tau[1])
                setOverrideRCIN(1500, 1500, 1500, yaw_pwm , 1500, sway_pwm)
            
            if motion == "yaw":
                # # #Desired position/velocity and acceleration :
                psid, rd, rd_d = quintic_Trajectory(psid_init, psid_final, current_time, t_final) 
                #psid, rd, rd_d, jerk = sept_Trajectory(psid_init, psid_final, current_time, t_final)
                Pos = np.array([0, 0, 0, 0, 0, psid])
                Speed_B = np.array([0, 0, 0, 0, 0, rd])
                Acc_B = np.array([0, 0, 0, 0, 0, rd_d])
                # Pos = np.array([0, 0, 0, 0, 0, first_joint_pos[cont]])
                # Speed_B = np.array([0, 0, 0, 0, 0, first_joint_vel[cont]])
                # Acc_B = np.array([0, 0, 0, 0, 0, first_joint_acc[cont]])
                # cont += 1 
                
                if cont == len(first_joint_pos):
                    cont -= 1
                tau = torque_rov(Acc_B, Pos, Speed_B)
                yaw_pwm = PWM_Cmd(tau[5])
            ...
            #     setOverrideRCIN(1500, 1500, 1500, yaw_pwm , 1500, 1500)
            # else :
            #     setOverrideRCIN(1500, 1500, 1500, yaw_pwm , 1550, 1500)
...
def setOverrideRCIN(channel_pitch, channel_roll, channel_throttle, channel_yaw, channel_forward, channel_lateral):
    ...
    msg_override.channels[3] = np.uint(
        channel_yaw)  # pulseCmd[5]  # yaw		    Lace
...
if __name__ == '__main__':
    ...
    robot_path = np.array([np.linspace(0, 1, 30),\
                                    np.linspace(0, 1, 30)]) # here assumes two joints
    my_robot = stopp.Robot(n_joints=robot_path.shape[0], j_max= 0.3, a_max=0.6, v_max=0.6, d_max=0.3) 
    #my_robot = stopp.Robot(n_joints=robot_path.shape[0], j_max= 0.5, a_max=0.6, v_max=1.5, d_max=0.1) 
    trajectory = my_robot.TimeParameterizePath(robot_path, interp_time_step=0.05)
    first_joint_trajectory = trajectory[0]
    #second_joint_trajectory = trajectory[1]
    first_joint_time = first_joint_trajectory.t
    first_joint_pos = first_joint_trajectory.pos
